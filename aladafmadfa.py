...
# --- محرك السحب الهجومي (نظام التحدي) ---
async def dragon_engine(sessions, src, trg, total, uid):
    found = []
    # مرحلة القنص (اختراق المخفي)
    ...
    try:
        ...
        return bot.send_message(uid, f"❌ فشل القنص: {e}")

    if not found:
        return bot.send_message(uid, "❌ المجموعة محصنة أو فارغة!")

    bot.send_message(uid, f"🐲 تم تجهيز `{len(found)}` هدف.\n⚔️ بدأت الغزوة بنظام التحدي...")
    ...
    for i, target in enumerate(found):
        ...
        current_s = sessions[i % len(sessions)]
        cl = TelegramClient(StringSession(current_s), MY_API_ID, MY_API_HASH)
        
        try:
            await cl.connect()
            # الهجوم المباشر باليوزر
            await cl(InviteToChannelRequest(channel=trg, users=[target.username]))
            success += 1
            bot.send_message(uid, f"🚀 تم سحب: `@{target.username}` ✅")
            await cl.disconnect()
            
            # سرعة "التحدي" (10 ثواني بين كل عضو)
            await asyncio.sleep(10)
            
        except (UserPrivacyRestrictedError, PeerFloodError):
            continue # تخطي لو العضو محمي أو الحساب تعب
        except Exception:
            continue

    bot.send_message(uid, f"🏁 **انتهت الغزوة!**\n✅ الإجمالي المجرور: `{success}`")

# --- الأوامر (بقية الكود كما هو بدون أي تغيير) ---
@bot.message_handler(func=lambda m: m.text == "🔄 بدء النقل (نظام دراجون)")
def flow_1(m):
    if get_balance(m.chat.id) < PRICE_PER_MEMBER: return bot.send_message(m.chat.id, "❌ رصيدك قليل.")
...
def handle_callbacks(call):
    ...
    if call.data == "pay_auto":
        ...
        bot.send_message(uid, f"📌 **USDT TRC20:**\n`{MY_WALLET}`\nأرسل صورة الإيصال 👇")
...
def wait_for_receipt(m):
    if m.photo:
        ...
        bot.send_photo(ADMIN_ID, m.photo[-1].file_id, caption=f"📩 طلب من `{m.chat.id}`", reply_markup=mk)
...
def del_acc_menu(m):
    ...
    bot.send_message(m.chat.id, "اختر حساباً:", reply_markup=mk)
