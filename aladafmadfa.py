...
API_ID = 2040
API_HASH = 'b18441a125727e640b07ff51f5463121'
ADMIN_ID = 5163375125  
PRICE_PER_MEMBER = 0.05 
OXAPAY_KEY = "CE8H0F-ISXBD2-RXHALY-KZXUZU"
MY_WALLET = "TLtLuhkU2kkkR1Wz1TtrBTpoNRTNviYpsA"

# مسار قاعدة البيانات (متوافق مع Volumes)
...
# تهيئة الجداول وتحميل الأرصدة للذاكرة لمنع التصفير
...
async def dragon_engine(uid, source, target, requested, mid):
    ...
    try:
        ...
        targets = []
        # سحب المدردشين (تحدي المجموعات المخفية)
        ...
        bot.edit_message_text(f"⚔️ تم صيد {len(targets)} هدف. بدأ الهجوم الفعلي...", uid, mid)
...
def calls(c):
    ...
    if c.data == "oxa_auto":
        ...
        else: bot.answer_callback_query(c.id, "⏳ لم يتم الدفع بعد.", show_alert=True)
    elif c.data.startswith("ok_"):
        _, amt, target = c.data.split("_")
        update_bal(int(target), float(amt))
        bot.send_message(int(target), f"✅ تم تأكيد شحن {amt}$")
...
def start(m):
    ...
    bot.send_message(m.chat.id, f"🐲 **مرحباً بك في دراجون V42**\n💰 رصيدك: `{GLOBAL_BALANCES.get(m.chat.id, 0.0):.2f}$`", reply_markup=main_kb())

@bot.message_handler(func=lambda m: m.text == "👤 حسابي")
def profile(m):
    bal = GLOBAL_BALANCES.get(m.chat.id, 0.0)
...
def step1(m):
    ...
    bot.register_next_step_handler(msg, step2, src)

def step2(m, src):
    trg = m.text
    msg = bot.send_message(m.chat.id, "🔢 العدد المطلوب:")
    bot.register_next_step_handler(msg, step3, src, trg)

def step3(m, src, trg):
    try:
        num = int(m.text)
        mid = bot.send_message(m.chat.id, "⏳ جاري التشغيل...").message_id
...
def add_acc1(m):
    bot.register_next_step_handler(bot.send_message(m.chat.id, "📱 الرقم مع المفتاح:"), add_acc2)
...
def add_acc3(m, p, h, s):
    ...
    if res == "OK": 
        ...
        bot.register_next_step_handler(bot.send_message(m.chat.id, "🔐 كلمة السر:"), add_acc4, p, fs)
...
def del_accs(m):
    ...
    bot.send_message(m.chat.id, "🗑️ تم.")
