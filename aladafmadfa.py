    ...
    try:
        ...
        if data and str(data.get('status')) in ['paid', 'success', '1']:
            ...
            if uid.isdigit():
                ...
                save_db()
                bot.send_message(uid, "🌟 **مبروك! تم تأكيد الدفع وتفعيل اشتراك VIP تلقائياً.**\nلديك الآن 5 تحليلات دقيقة يومياً لمدة 30 يوم.")
...
def get_analysis(symbol):
    try:
        ...
        p = closes[-1]; sma = sum(closes[-20:]) / 20
        ...
        if rsi < 35 and p <= sma:
            sig, stat = "🚀 شراء (منطقة ارتداد ذهبية)", "المؤشرات تشير لقاع فني مشبع بالبيع."
            t1, t2, sl = p*1.045, p*1.09, sup*0.97
        elif rsi > 65 or p >= resis:
            sig, stat = "⚠️ منطقة جني أرباح (خطر)", "السعر متضخم عند مقاومة قوية."
            t1, t2, sl = p*1.01, p*1.02, p*0.99
        else:
            sig, stat = "📈 اتجاه صاعد مستقر", "الزحم معتدل والترند إيجابي."
...
def ana_execute(m):
    ...
    bot.send_message(m.chat.id, "⏳ جاري فحص المؤشرات...")
...
def pay_final(m):
    ...
    amount = m.text
    bot.send_message(m.chat.id, "⏳ جاري تجهيز فاتورة الدفع...")
    
    # محاولة الاتصال بـ OxaPay لتوليد فاتورة تفعيل تلقائي
    try:
        payload = {
        ...
        if r.get("payUrl"):
            markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("💳 دفع الآن (تفعيل تلقائي)", url=r['payUrl']))
            bot.send_message(m.chat.id, f"✅ **تم تجهيز فاتورتك بنجاح**\n💰 المبلغ: {amount}$\n\nسيتم تفعيل حسابك آلياً فور الدفع.", reply_markup=markup)
            return
    except: pass

    # الحل البديل (المنقذ) في حالة فشل الاتصال أو ضغط السيرفر
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("💳 دفع الآن (رابط مباشر)", url=BACKUP_PAY_LINK))
    markup.add(types.InlineKeyboardButton("👨‍💻 تفعيل يدوي / دعم", url=f"tg://user?id={OWNER_ID}"))
    bot.send_message(m.chat.id, f"⚠️ **بوابة الدفع التلقائي مزدحمة.**\n\nلكن يمكنك الدفع عبر الرابط الاحتياطي أدناه:\n💰 المبلغ: {amount}$\n\n**ملاحظة:** بعد الدفع، أرسل صورة الإيصال ليتم تفعيلك يدوياً.", reply_markup=markup)
...
def acc_info(m):
    ...
    if is_v:
        ...
        msg = f"🌟 **الحالة: VIP نشط**\n📅 المتبقي: {int((db['vip'][uid]-time.time())/86400)} يوم\n📈 استهلاك اليوم: {c}/5"
    else:
...
def handle_receipt(m):
    ...
    bot.send_message(m.chat.id, "⏳ تم استلام الصورة، جاري المراجعة.")
