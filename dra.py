...
import json
from telethon.tl.functions.messages import GetMessagesReactionsRequest, GetHistoryRequest
...
def get_balance(uid):
    ...
    try:
        response = supabase_client.table("users_dragon").select("balance").eq("uid", int(uid)).execute()
        if response.data and len(response.data) > 0:
            return round(float(response.data[0]["balance"]), 3)
        else:
            supabase_client.table("users_dragon").insert({"uid": int(uid), "balance": 0.0}).execute()
            return 0.0
...
def update_balance(uid, amt):
    ...
    try:
        ...
        supabase_client.table("users_dragon").update({"balance": new_balance}).eq("uid", int(uid)).execute()
...
def save_user_memory(user_id):
    if not supabase_client: return
    try:
        supabase_client.table("memory_dragon").upsert({"target_id": str(user_id)}).execute()
    except Exception as e:
		...
		try:
			...
			for t in targets:
				...
				try:
					...
					success += 1; count += 1
					bot.send_message(uid, f"➕ [{session_file}] أضاف:\n\n{t.first_name}\n\n")
...
def start_main(m):
    ...
    if len(params) > 1 and params[1].isdigit():
        ...
        if ref_id != uid:
            ...
            try: bot.send_message(ref_id, f"🎉 **مبارك!** دخل عضو جديد برابطك، كسبت `{REFERRAL_GIFT}`$.")
            except: pass
    ...
    if uid == ADMIN_ID: mk.add("⚙️ لوحة الأدمن")
    bot.send_message(uid, "🐉 **مرحباً بك في بوت دراغون المحرك V73**\nأهلاً بك في أقوى منصة كاشفة وساحبة للأعضاء الحقيقيين والمتفاعلين.. سيزيد ترتيب مجموعتك الآن .", reply_markup=mk)

@bot.message_handler(func=lambda m: m.text == "🎁 كسب رصيد مجاني")
def referral_menu(m):
    ref_link = f"https://t.me/{bot.get_me().username}?start={m.chat.id}"
    bot.send_message(m.chat.id, f"🎁 **نظام الإحالة كالتالي:**\nانشر رابطك واكسب رصيد مجاني عن كل مستخدم يقيد حساباته:\n`{ref_link}`")
...
def payment_menu(m):
    ...
    bot.send_message(m.chat.id, f"💵 رصيدك الحالي هو: `{get_balance(m.chat.id)}`$", reply_markup=mk)
...
def process_oxa(m):
    ...
    try:
        ...
        if pay_url:
            ...
            bot.send_message(m.chat.id, f"⏳ جاري تجهيز فاتورة بقيمة {amt}$ (دفع تلقائي):\n🔗 سيتم فحص دفعك دورياً بمجرد إرسال الدفع.", reply_markup=markup)
...
def auto_check_payment(chat_id, track_id, amount):
    for _ in range(60):
        ...
        try:
            ...
            if check.get('status') == 'Paid' or check.get('result') == '100' or check.get('result') == 100:
                update_balance(chat_id, amount)
                bot.send_message(chat_id, f"🎉 **مبارك!** تم استقبال الدفع التلقائي للمبلغ.\n💵 تم إضافة `{amount}$` إلى رصيدك بنجاح.")
                break
        except: continue
...
def handle_receipt(m):
    if user_states.get(m.chat.id) == "waiting_receipt":
        ...
        bot.send_photo(ADMIN_ID, m.photo[-1].file_id, caption=f"📩 وصل شحن جديد\n👤 للمستثمر: `{m.chat.id}`", reply_markup=mk)
        bot.reply_to(m, "⏳ تم استلام إيصال الدفع، جاري مراجعته والتحقق منه من قبل الإدارة...")
...
def admin_confirm(c):
    try:
        ...
        bot.send_message(int(uid), f"🎉 **مبارك!** تم قبول إيصالك وشحن رصيدك بمبلغ {amt}$ بنجاح.")
        bot.edit_message_caption(f"✅ تم قبول الشحن بنجاح للمستخدم {uid} بمبلغ ({amt}$)", c.message.chat.id, c.message.message_id)
    except Exception as e:
...
def process_phone(m):
    ...
    if not ph.isdigit(): return bot.send_message(m.chat.id, "⚠️ الرقم غير صحيح.")
    ...
    except Exception as e: bot.send_message(m.chat.id, f"⚠️ عذراً: {str(e)}")
    finally: loop.close()
...
def process_password(m, sess, ph):
    ...
    try:
        if loop.run_until_complete(sign_p()) == "OK":
            bot.send_message(m.chat.id, "✅ **تم ربط الحساب بنجاح!**")
            save_account_db(m.chat.id, sess, ph)
        else: bot.send_message(m.chat.id, "❌ خطأ في رمز التحقق بخطوتين.")
    finally: loop.close()
...
def delete_acc_menu(m):
    army = [f for f in os.listdir('.') if f.startswith(f"sess_{m.chat.id}_") and f.endswith('.session')]
    if not army: return bot.send_message(m.chat.id, "❌ لا توجد حسابات مربوطة.")
...
def finalize_delete(c):
    ...
    try:
        ...
        if supabase_client:
            supabase_client.table("accounts_dragon").delete().eq("session_name", fname).execute()
        bot.answer_callback_query(c.id, "✅ تم الحذف بنجاح")
        bot.edit_message_text(f"✅ تم إلغاء ربط الحساب وحذفه `{fname.split('_')[-1]}`.", c.message.chat.id, c.message.message_id)
    except Exception as e: bot.answer_callback_query(c.id, f"❌ حدث خلل: {str(e)}")
...
def info(m):
    ...
    army_count = len([f for f in os.listdir('.') if f.startswith(f"sess_{m.chat.id}_") and f.endswith('.session')])
