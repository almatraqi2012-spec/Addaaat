...
# --- [ محرك السحب المعدل - التحدي الحقيقي ] ---
...
def final_check(m, src, trg):
    try:
        ...
        if balance < cost:
            return bot.send_message(m.chat.id, f"❌ رصيدك غير كافي! التكلفة: {cost}$")
        ...
        bot.send_message(m.chat.id, f"⚔️ **بدأ الهجوم الآن... جاري سحب {count} عضو بتحدي!**")
...
async def run_dragon(sessions, src, trg, total, uid):
    ...
    try:
        ...
        # سحب الأعضاء المتفاعلين فقط (لضمان نجاح الإضافة)
    ...
    if not found:
        return bot.send_message(uid, "❌ لم يتم العثور على أهداف (المجموعة محمية).")
    ...
    for i, target in enumerate(found):
        ...
        try:
            ...
            await cl.disconnect()
            await asyncio.sleep(6) # فاصل هجومي سريع
        except (UserPrivacyRestrictedError, UserNotMutualContactError):
            continue # تخطي العضو لو الخصوصية قوية
        except (PeerFloodError, FloodWaitError):
            continue # الحساب تعب، ننتقل للي بعده
        except Exception:
            continue
    
    bot.send_message(uid, f"🏁 **انتهت الغزوة!**\n✅ الإجمالي المضاف فعلياً: `{success}`")

# --- [ باقي الكود (شحن، حذف، إضافة) يظل كما هو ] ---
# (استخدم نفس الدوال الموجودة في كودك الأصلي هنا بدون تغيير)
...
def wait_for_receipt(m):
    if m.photo:
        mk = types.InlineKeyboardMarkup()
        mk.add(types.InlineKeyboardButton("✅ تفعيل 10$", callback_data=f"adm_confirm_10_{m.chat.id}"))
